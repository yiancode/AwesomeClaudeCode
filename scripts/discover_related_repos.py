...
def get_forks(owner: str, repo: str, token: Optional[str] = None, limit: int = 30) -> List[dict]:
    ...
    params = {"sort": "stargazers", "per_page": min(100, limit)}
...
def get_stargazers_also_starred(
    owner: str, repo: str, token: Optional[str] = None, sample_size: int = 20
) -> List[dict]:
    """
...
def filter_related_repo(repo: dict, config: dict, existing_urls: Set[str], is_fork: bool = False) -> Tuple[bool, str]:
    """
...
def create_candidate_from_repo(
    repo: dict, source_resource: dict, relation_type: str, config: dict, categories_prefix: dict, relevance_score: int
) -> dict:
...
def main():
    ...
    parser.add_argument(
        "--type", choices=["forks", "similar", "all"], default="all", help="Type of relation to discover"
    )
    ...
    for repo, source_resource, relation_type, score in unique_candidates:
        resource = create_candidate_from_repo(repo, source_resource, relation_type, config, categories_prefix, score)
